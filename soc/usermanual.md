...
```c
    ...
    dtof_init_and_wait_for_ready(device_id, &chip_id, DTOF_FALSE);
...
void main() {
    ...
    dtof_init_and_wait_for_ready(0,&chip_id,DTOF_FALSE);
    dtof_init_and_wait_for_ready(1,&chip id,DTOF_FALSE);
    ...
    while(1){
        ...
        PrintfTask();
        do {
            dtof_get_distance_result(0, &distance_result_front, &is_new_flag)
...
5.4.1 移动sensor到距离物体20mm处
5.4.2 注册步骤4中读写flash的接口
5.4.3 调用测试api
```c
dtof_init_and_wait_for_ready(device_id, &chip_id, DTOF_TRUE);
```
