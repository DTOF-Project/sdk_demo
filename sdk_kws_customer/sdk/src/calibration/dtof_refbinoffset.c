...
static DTOF_RET dtof_refbinoffset_set(uint16_t offset)
{
    DTOF_CHECK_RET(dtof_reg_burst_write(device_id, DTOF_REG226, &offset, 1),
              "设置参考bin偏移失败");
    return DTOF_RET_SUCCESS;
}

static void dtof_find_peak_16(const uint16_t *data,
                             uint32_t length,
                             dtof_peak_info_t *peak)
{
...
static void dtof_find_peak_32(const uint32_t *data,
                             uint32_t length,
                             dtof_peak_info_t *peak)
{
    DTOF_CHECK_PTR_VOID(data && peak && length > 0);
...
static uint16_t dtof_get_average_peak(void)
{
    ...
    for(uint32_t frame = 0; frame < DTOF_REF_BIN_FRAME_COUNT; frame++) {
        DTOF_CHECK_RET(dtof_calibration_get_frame_data(
                     DTOF_REF_RAM_START_ADDR,
                     current_frame,
                     DTOF_REF_BIN_BUFFER_SIZE),
                 "获取帧数据失败");
...
uint16_t dtof_maxrefbin_find(void)
{
    ...
    for(uint16_t offset = 0; offset < DTOF_REF_BIN_TOTAL_LENGTH; offset += DTOF_REF_BIN_OFFSET_STEP) {
        // 设置当前offset
        dtof_refbinoffset_set(offset);
        // 先做单帧测试
        uint16_t test_frame[DTOF_REF_BIN_BUFFER_SIZE];
        dtof_peak_info_t test_peak = {0, 0};
        dtof_calibration_get_frame_data(DTOF_REF_RAM_START_ADDR, test_frame, DTOF_REF_BIN_BUFFER_SIZE);
...
DTOF_RET dtof_calibration_refbinoffset_calculate(
    binoffset_cal_t *binoffset_cal_data_p)
{
    DTOF_CHECK_PARAM(binoffset_cal_data_p, "校准数据指针为空");

    uint16_t best_pos = dtof_maxrefbin_find();
    ...
    DTOF_CHECK_RET(dtof_refbinoffset_set(best_pos),
              "设置最佳偏移值失败");

    // 多帧确认
    uint16_t avg_pos = dtof_get_average_peak();

    // 计算最终偏移值
    binoffset_cal_data_p->ref_cal_mp1 = best_pos + avg_pos;
    binoffset_cal_data_p->binoffset =
        best_pos + avg_pos - DTOF_REF_BIN_OFFSET_SUB;

    DTOF_CHECK_RET(dtof_refbinoffset_set(binoffset_cal_data_p->binoffset),
              "设置最终偏移值失败");
