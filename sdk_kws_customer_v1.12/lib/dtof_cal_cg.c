...
#define DTOF_VAL_SAMPLE_MULTIPLE_OBJECT   11
...
DTOF_RET dtof_cross_talk_sample_data(uint8_t device_id, uint16_t frame_size, uint16_t *cross_talk_calib_data)
{
    ...
    dtof_find_max_uint16(cross_talk_calib_data, DTOF_CG_PEAK_MAX_INDEX, &cg_peak_pos);

#ifdef DTOF_CT_DEBUG
    printf("cg_front_mean:%d\n", cg_front_mean);
    printf("object_peak_pos:%d, cg_peak_pos:%d\n", object_peak_pos, cg_peak_pos);
#endif
    ...
    for (int i = 0; i < DTOF_CT_BIN_SIZE_NEW; i++)
    {
        cross_talk_calib_data[i] = cross_talk_calib_data[i] * DTOF_VAL_SAMPLE_MULTIPLE_OBJECT / DTOF_VAL_BOOST_MULTIPLE;
    }

    dtof_find_min_uint16(cross_talk_calib_data + cg_peak_pos, object_peak_pos - cg_peak_pos + 1, &min_pos_between_two_peak);
#ifdef DTOF_CT_DEBUG
...
//     {
//         temp_sample_data_v[i] = value_ceil(cross_talk_histgram[i] * maxfls, DTOF_CG_CALIB_FLASH_CNT);
//     #ifdef DTOF_CT_DEBUG
//         printf("%d, ", temp_sample_data_v[i]);
//     #endif
//         // temp_sample_data_v[i] = cross_talk_histgram[i] * maxfls / (DTOF_CG_CALIB_FLASH_CNT);
//     }
// #ifdef DTOF_CT_DEBUG
//     printf("\n");
// #endif
...
DTOF_RET hal_next_ac_data_config(uint8_t device_id, dtof_uint16_t *next_ac_data_p, dtof_uint16_t length)
{
    DTOF_RET ret;
...
DTOF_RET hal_next_dc_data_config(uint8_t device_id, dtof_uint16_t next_dc_data)
{
    DTOF_RET ret;
...
DTOF_RET dtof_do_cross_talk_calibration(uint8_t device_id, uint16_t *ct_reg_data)
{
    ...
    printf("calc cross talk calibration\n");
