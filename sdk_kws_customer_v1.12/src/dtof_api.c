...
static dtof_get_distance_result_t get_distance_result[DTOF_MAX_DEVICE_NUM] =
{
    {.device_id = 0, .first_frame = 1, .frame_id_pre = 0, .distance_offset = 0},
    {.device_id = 1, .first_frame = 1, .frame_id_pre = 0, .distance_offset = 0},
};

void dtof_set_distance_offset(dtof_uint8_t device_id, dtof_int32_t offset)
{
    get_distance_result[device_id].distance_offset = offset;
}

dtof_int32_t dtof_get_distance_offset(dtof_uint8_t device_id)
{
    return get_distance_result[device_id].distance_offset;
}
...
DTOF_RET dtof_get_fifo(dtof_uint8_t device_id, dtof_start_mode_t dtof_start_mode, dtof_distance_result_t* result_info_p)
{
    ...
    result_info_p->first_target = (dtof_int16_t)(distance_result[DTOF_FIFO_DISTANCE0] / DTOF_DISTANCE_RESULT_DIVISOR - DTOF_DISTANCE_RESULT_OFFSET + get_distance_result[device_id].distance_offset);
...
DTOF_RET dtof_get_distance_result(dtof_uint8_t device_id, dtof_start_mode_t dtof_start_mode, dtof_distance_result_t* result_info_p, dtof_bool_t *is_new_frame)
{
    ...
    *is_new_frame = DTOF_FALSE;
    ...
    if (get_distance_result[device_id].first_frame) {
        // 第一帧不要
        get_distance_result[device_id].first_frame = 0;
        get_distance_result[device_id].frame_id_pre = frame_id;
        return DTOF_RET_SUCCESS;
    } else {
        // 检查帧ID是否变化
        if (frame_id == get_distance_result[device_id].frame_id_pre) {
            // 同一帧
            return DTOF_RET_SUCCESS;
        }

        if (((frame_id - get_distance_result[device_id].frame_id_pre) != 1) && ((frame_id - get_distance_result[device_id].frame_id_pre) != 2)) {
            // 异常帧
            get_distance_result[device_id].first_frame = 1;
...
#define DTOF_INIT_FRAME_ID 2
    ...
    dtof_uint16_t frame_id;
    dtof_uint16_t reset_value = 0x27ea;
    ...
    // reset mcu
    do{
        ret = dtof_reg_burst_write(device_id, DTOF_IO_CTRL_REG_ADDR, &reset_value, 1);
        if (ret != DTOF_RET_SUCCESS){
            burn_try_count++;
        }
        dtof_sleep_ms(1);
    ...
    do{
        ...
        dtof_sleep_ms(DTOF_INIT_DELAY);
    } while (frame_id < DTOF_INIT_FRAME_ID);
    ...
    switch(dtof_start_mode)
    {
        ...
        case DO_XTALK_CALIBRATION_MODE:
        {
            ...
            printf("xtalk data:\n");
            for(dtof_uint8_t i = 0; i < XTALK_DATA_SIZE; i++)
            {
...
#define DTOF_DO_CALIBRATION_MAX_FRAME 200
    ...
    dtof_uint32_t total_frame_cnt = 0;
    ...
    while (total_frame_cnt < DTOF_DO_CALIBRATION_MAX_FRAME)
    {
        ...
        if (distance_result.is_legal_frame != 1)
            continue;

        legal_frame_cnt++;

        if (legal_frame_cnt > (DTOF_DO_CALIBRATION_IGNORE_FRAME + DTOF_DO_CALIBRATION_SAMPLE_FRAME))
        {
            break;
        }
    ...
    if (legal_frame_cnt <= (DTOF_DO_CALIBRATION_IGNORE_FRAME + DTOF_DO_CALIBRATION_SAMPLE_FRAME)) {
        DTOF_LOG_ERR("calibration failed: not enough valid frames, %u\n", legal_frame_cnt);
        return DTOF_RET_FAILED;
    }
